...
#import list of all hp directories

hp = open('/beegfs/lofar/lclews/DR2_ridgelines/full_sample_2/hp_directories.txt').read().splitlines()
number_of_hp = len(hp)
print('There are', number_of_hp, 'healpix directories')

batch_number = int(sys.argv[1])
path = hp[batch_number]

print('Batch number =',batch_number)
print('Healpix number=', os.path.basename(hp[batch_number]).split('_')[1])


#import sources in this hp dir

all_names = open('/beegfs/lofar/lclews/DR2_ridgelines/full_sample_2/Data/all_names.txt').read().splitlines()
dirs_for_names = open('/beegfs/lofar/lclews/DR2_ridgelines/full_sample_2/hp_directories_all_sources.txt').read().splitlines()
...
for i in catalogue:
    ...
    if source_name in names:
       ...
       # df.to_csv('/beegfs/lofar/lclews/DR2_ridgelines/full_sample_2/SB_csv/%s-SB.csv' %source_name)
        ...
        plt.legend() 
        plt.savefig('/beegfs/lofar/lclews/DR2_ridgelines/full_sample_2/u_cord/Profile-%s.png' %source_name)
        plt.show()
       ...
       # df.to_csv('/beegfs/lofar/lclews/DR2_ridgelines/full_sample_2/ridge_spline_csv/%s-ridge_spline.csv' %source_name, )
        ...
        np.savetxt('/beegfs/lofar/lclews/DR2_ridgelines/full_sample_2/u_test/%s-curvature.txt' %source_name, kappa, delimiter=' ')

        plt.figure(figsize=(10,8))
        plt.plot(u_c,kappa,'-', color='k', label='Curvature')
        plt.axvline(host_cord, ls='--', color='b', label='Host Position')
        plt.title(source_name)
        plt.xlabel('u_c')
        plt.ylabel('$\kappa$')
        plt.savefig('/beegfs/lofar/lclews/DR2_ridgelines/full_sample_2/u_test/%s-curavture.png' %(source_name))
        plt.show()

        #curvature/spline plot
        fig, (ax1,ax2) = plt.subplots(1,2, figsize=(10,8))
        ax2.set_xlabel('u_test')
        ax2.set_ylabel('$\kappa$')
        ax1.set_xlabel('RA (pix)')
        ax1.set_ylabel('DEC (pix)')
        ax1.plot(Ix,Iy, 'x', color='b', label='Optical Host')
        plt.suptitle(source_name)
        ax2.plot(u_c,kappa,'-', color='k', label='Curvature')
        ...
        ax1.legend()
        plt.savefig('/beegfs/lofar/lclews/DR2_ridgelines/full_sample_2/u_test/%s-curvature_and_ridge.png' %(source_name))
