...
for i in catalogue[0:30]:
    ...
    try:
        file1,file2 = get_ridge_txt_files(source_name)
        file1 = open(file1[0])
        file2 = open(file2[0])
    except:
    ...
    fits_cutout = get_fits_cutout(source_name)
    hdu = fits.open(fits_cutout[0])
    w = WCS(hdu[0].header)
    hostlength, ArcHost, Ix,Iy = find_host_position(source_name, hostx_deg, w,points,lengths)
